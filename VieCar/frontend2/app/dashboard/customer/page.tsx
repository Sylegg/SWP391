...
export default function CustomerDashboard() {
  // Import loadUserFromToken từ AuthContext để xử lý Google OAuth callback
  const { user, updatePreferredDealer, loadUserFromToken } = useAuth();
  const { toast } = useToast();
  const router = useRouter();
  const searchParams = useSearchParams(); // Hook để đọc URL query parameters
   ...
   * 
   * UseEffect này xử lý redirect từ backend sau khi Google OAuth thành công.
   * Backend redirect về dashboard với tokens trong URL query params.
   * 
   * Flow:
   * 1. User click "Sign in with Google" button (trong login page)
   * 2. Frontend redirect đến /oauth2/authorization/google
   * 3. Google xác thực user
   * 4. Backend callback nhận user info từ Google, tạo JWT tokens
   * 5. Backend redirect về đây: /dashboard/customer?google_login=success&token=xxx&refreshToken=xxx&role=xxx
   * 6. useEffect này detect URL params và xử lý:
   *    - Lưu tokens vào localStorage
   *    - Load user info từ token (không gọi login API vì không có password)
   *    - Hiển thị toast thông báo
   *    - Clean URL params (remove sensitive info khỏi URL)
   * 
   * Dependency array [searchParams, loadUserFromToken, router, toast]:
   * - Effect chỉ chạy lại khi URL params thay đổi (khi redirect từ backend)
   */
  useEffect(() => {
    // Đọc query parameters từ URL
    const googleLogin = searchParams.get('google_login');  // Flag báo đây là Google OAuth callback
    const token = searchParams.get('token');                // JWT access token
    const refreshToken = searchParams.get('refreshToken'); // Refresh token
    const role = searchParams.get('role');                  // User role

    // Kiểm tra nếu có đủ params từ Google OAuth callback
    if (googleLogin === 'success' && token && refreshToken && role) {
      // Lưu tokens vào localStorage để persist authentication state
      // Frontend sẽ dùng token này để authenticate các API calls tiếp theo
      localStorage.setItem('token', token);              // Access token (JWT)
      localStorage.setItem('refreshToken', refreshToken); // Refresh token để renew JWT
      localStorage.setItem('role', role);                 // User role

      // Load user info từ token có sẵn (KHÔNG gọi login API)
      // Lý do: Google user không có password, nếu gọi POST /api/auth/login sẽ bị 401
      // loadUserFromToken() sẽ:
      // - Decode JWT để lấy username
      // - Gọi getUserProfile() để lấy full user info từ backend
      // - Update AuthContext state với user info
      loadUserFromToken();

      // Hiển thị toast notification để báo user đã login thành công
      toast({
      ...
      // Clean URL params: remove tokens khỏi URL để:
      // 1. Tránh expose sensitive tokens trong browser history
      // 2. Prevent useEffect chạy lại nếu user refresh page
      // router.replace() thay đổi URL mà không tạo history entry mới
                      ...
                      {dealersWithDistance.map((dealer, index) => (
                        ...
                        >
                          ...
                          <div className="relative flex items-start gap-4">
                            {/* Ranking Badge */}
                            {index < 3 && (
                              <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shadow-md ${index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-yellow-900' :
                            ...
                            {index >= 3 && (
                              <div className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center bg-gradient-to-br from-emerald-100 to-teal-100 border border-emerald-200">
                                <MapPin className={`h-4 w-4 transition-all ${selectedDealer === dealer.id.toString() ? 'text-emerald-600' : 'text-gray-400 group-hover:text-emerald-500'
                                  }`} />
                              </div>
                            )}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-3 mb-2">
                                <h3 className={`font-bold text-base transition-colors ${selectedDealer === dealer.id.toString() ? 'text-emerald-700' : 'text-gray-900'
                                  }`}>
                                  {dealer.name}
                                </h3>
                                {selectedDealer === dealer.id.toString() && (
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 flex-wrap">
                                  ...
                                  {dealer.distance && (
                                    <Badge
                                      variant="outline"
                                      className={`text-xs font-semibold ${selectedDealer === dealer.id.toString()
                    ...
                    <div className="space-y-3 pt-4 animate-[scale-in_0.3s_ease-out]">
                      ...
                      <div className="border-2 border-emerald-200 rounded-xl overflow-hidden relative shadow-lg" style={{ height: '400px' }}>
                        <iframe
                          ...
                          src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8&q=${selectedDealer && dealers.find(d => d.id === parseInt(selectedDealer))?.latitude
