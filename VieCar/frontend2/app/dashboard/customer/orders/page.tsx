...
export default function CustomerOrdersPage() {
  ...
  const getStatusIcon = (status: string) => {
    const iconMap: Record<string, JSX.Element> = {
      'Chờ xử lý': <Clock className="h-5 w-5 text-yellow-600" />,
      'Chưa đặt cọc': <CreditCard className="h-5 w-5 text-orange-600" />,
      'Đã đặt cọc': <CheckCircle className="h-5 w-5 text-blue-600" />,
      'Đang chuẩn bị xe': <Truck className="h-5 w-5 text-cyan-600" />,
      'Đã yêu cầu đại lý': <Truck className="h-5 w-5 text-indigo-600" />,
      'Sẵn sàng giao xe': <Car className="h-5 w-5 text-purple-600" />,
      'Đã giao': <Package className="h-5 w-5 text-green-600" />,
      'Đã hủy': <AlertCircle className="h-5 w-5 text-red-600" />,
    };
    return iconMap[status] || <FileText className="h-5 w-5 text-gray-600" />;
  };
