...
import com.lemon.supershop.swp391fa25evdm.testdrive.model.dto.response.AvailabilityCheckRes;
import com.lemon.supershop.swp391fa25evdm.testdrive.model.dto.response.AvailableSlotsRes;
...
import com.lemon.supershop.swp391fa25evdm.testdrive.repository.TestDriveRepo;
...
public class TestDriveService {
    ...
    @Autowired
    private TestDriveRepo testDriveRepo;
    @Autowired
    ...
    public List<TestDriveRes> getTestDriveByUserId(int userId) {
        List<TestDrive> testDrives = testDriveRepo.findByUserId(userId);
        return testDrives.stream().map(this::convertToRes).toList();
    }

    public List<TestDriveRes> getTestDriveByDealerId(int dealerId) {
        List<TestDrive> testDrives = testDriveRepo.findByDealerId(dealerId);
        return testDrives.stream().map(this::convertToRes).toList();
    }
    ...
    public TestDriveRes updateTestDrive(int id, TestDriveReq req) {
        Optional<TestDrive> testDrive = testDriveRepo.findById(id);
    ...
    public TestDriveRes assignVehicleAndStaff(int testDriveId, int productId, int escortStaffId) {
        ...
        List<TestDrive> conflicts = testDriveRepo.findConflictingTestDrives(
        ...
        TestDrive savedTestDrive = testDriveRepo.save(testDrive);
...
//        }
    ...
    public AvailableSlotsRes getAvailableSlots(
        ...
        try {
            ...
            List<TestDrive> bookedSlots = testDriveRepo.findByProductAndDate(productId, dateTime);
    ...
    public boolean deleteTestDrive(int id) {
        if (testDriveRepo.findById(id).isPresent()) {
            testDriveRepo.deleteById(id);
            return true;
        }
        return false;
    }
    ...
    private TestDrive convertToEntity(TestDrive testDrive, TestDriveReq req) {
        if (testDrive != null || req != null) {
            ...
            if (req.getStatus() != null && testDrive.getStatus() != null) {
...
//            Check if user has already completed test drive for this category
    ...
    public String generateIcsFile(int testDriveId) {
        ...
        if (testDriveId > 0) {
            Optional<TestDrive> testDrive = testDriveRepo.findById(testDriveId);
            if (!testDrive.isPresent()) {
